...
function filterMD(arr) {
    return (
        arr.reduce((result, item) => {
            result.push({
                date: item.release_date, poster: item.poster_path, title: item.title,
                excerpt: item.overview, overview: item.overview, id: item.id, rate: item.vote_average,
                bgimg: item.backdrop_path, genres: item.genre_ids, singleMovieGenres: item.genres, runtime: item.runtime, tagline: item.tagline
            });
            return result;
        }, [])
    )
...
function setRuntime(obj) {
    if (obj.runtime !== null && obj.runtime !== 0) {
        obj.runtime = obj.runtime + ' Minutes';
    } else {
        obj.runtime = ''
    }
}
